...
String sel = request.getParameter("select_brand");
   String sel1 = request.getParameter("select_engine");
	 Statement stmt = null;
	 //ResultSet rs=null;
	 ResultSet rs1=null;
	 try
		{
		   ...
		   session.putValue("e_type",sel1);
		  // PreparedStatement ps=con.prepareStatement("select distinct model_name  from vehicle natural join models where brand_name=?");
		   //ps.setString(1,sel);
		    //rs=ps.executeQuery();
		    PreparedStatement ps1=con.prepareStatement("select distinct transmission_type  from  options natural join vehicle where brand_name=? and engine_type=?");
			   ps1.setString(1,sel);
			   ps1.setString(2,sel1);
			    rs1=ps1.executeQuery();
			    //System.out.println(sel1);
		    out.println("<h1>The transmission type for your selected engine type is/are :  " + "</h1><br>");
		    while(rs1.next())
			{
			String t_type=rs1.getString("transmission_type");
			session.putValue("t_type",t_type);
			//System.out.println(mo_name);
			out.println("<h1>"+t_type +"</h1><br>");				//session.putValue("model_id",m_id);
   ...
   %>
   <form action=choose_car5.jsp method="post"><center>
