...
String sel = request.getParameter("select_brand");
   String sel1 = request.getParameter("select_color");
	 Statement stmt = null;
	 //ResultSet rs=null;
	 ResultSet rs1=null;
	 try
		{
		   ...
		   session.putValue("c_name",sel1);
		  // PreparedStatement ps=con.prepareStatement("select distinct model_name  from vehicle natural join models where brand_name=?");
		   //ps.setString(1,sel);
		    //rs=ps.executeQuery();
		    PreparedStatement ps1=con.prepareStatement("select distinct engine_type  from  options natural join vehicle where brand_name=? and color_name=?");
			   ps1.setString(1,sel);
			   ps1.setString(2,sel1);
			    rs1=ps1.executeQuery();
			    //System.out.println(sel1);
		    out.println("<h1>The engine type for your selected color is/are :  " + "</h1><br>");
		    while(rs1.next())
			{
			String e_type=rs1.getString("engine_type");
			//System.out.println(mo_name);
			out.println("<h1>"+e_type +"</h1><br>");				//session.putValue("model_id",m_id);
   ...
   %>
   <form action=choose_car4.jsp method="post"><center>
